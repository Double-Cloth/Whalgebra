    ...
    class VirtualScroll {
        ...
        _render(force = false) {
            ...
            try {
                ...
                // 渲染失败后统一清理现有节点，并重置窗口范围，
                // 避免留下部分节点或让下一次渲染被 rangeUnchanged 错误跳过。
                this._recycleRenderedNodes();

                // 恢复为不依赖任何已渲染节点的完整占位结构，
                // 保证滚动高度与当前空渲染窗口保持一致。
                if (this._spacerTop?.isConnected && this._spacerBottom?.isConnected) {
                    const {top, bottom} = this._heightMapper.calcSpacerHeights(
        ...
        _incrementalUpdate(prevStart, prevEnd, newStart, newEnd) {
            ...
            try {
                ...
                // 新节点尚未写入 _renderedNodes，发生异常时必须单独回收，
                // 否则对象池会保留无法再访问的“使用中”节点引用。
        ...
        load(data, length, {remeasure = false, resetScroll = true} = {}) {
            ...
            try {
                ...
                // load() 可能已经修改数据、DOM、监听器或测量状态，
                // 无法可靠恢复调用前现场，因此统一清理并回到可重新 load() 的 IDLE 状态。
