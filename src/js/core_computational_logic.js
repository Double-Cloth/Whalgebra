...
(function () {
     ...
     * @property {Array<string|'error'>} parameter - 按模型表达式顺序排列的参数；二参数模型为
     *   `[a,b]`，二次模型为 `[a,b,c]`。无法求解的参数逐项使用 `error`。
     ...
     * @property {Array<[string, string]>} increasingInterval - 单调递增区间；不存在时为
     *   `[['null','null']]`，无穷端点使用专用字符串。
     ...
     * @property {Array<string>} roots - 已格式化实根/复根列表；无根为 `['null']`，恒等于零为
     *   `['anyRealNumber']`。返回数量不超过实际多项式次数。
     */
    ...
    class FuncValueListTools {
         ...
         *
         * @param {string} f - 使用内部词元书写的 `f(x)` 函数体；可通过 `[g]` 调用 `g`，单点求值
         *   失败只在 `f` 结果的同一位置写入 `error`。
         * @param {string} g - 使用内部词元书写的 `g(x)` 函数体；可通过 `[f]` 调用 `f`，其失败位置
         *   与 `f` 独立记录。
         ...
         * @param {string|number|BigNumber|ComplexNumber|Array} end - 闭区间实数终点；仅计算按步长实际
         *   到达且不大于它的点，不会为了包含终点而改变最后一步。
