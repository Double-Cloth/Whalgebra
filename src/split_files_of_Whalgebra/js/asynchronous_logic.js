...
class SyncWorker {
    ...
    _generateWorkerCode(funcs, deps) {
        ...
        const serializeValue = (value) => {
            // 如果值是一个函数，则将其转换为其源代码的字符串表示。
            // 例如，`() => console.log('hello')` 会变成 ` "() => console.log('hello')" `。
            if (typeof value === 'function') {
                return value.toString();
            }

            // `JSON.stringify(undefined)` 会返回 `undefined` 而不是字符串 `"undefined"`。
            if (value === undefined) {
                return 'undefined';
            }

            // 注意: JSON.stringify 无法处理循环引用的对象（会导致错误）
            // 或某些特殊对象，如 Map, Set, RegExp, Date 等（它们不会被正确序列化）。
            // 对于包含复杂依赖或特殊对象的场景，建议在序列化之前将其分解或转换为可序列化的部分。
