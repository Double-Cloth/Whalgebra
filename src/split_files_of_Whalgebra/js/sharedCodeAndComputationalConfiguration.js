"use strict";
        ...
        class Public {
            ...
            static idealizationToString(x, {
                ...
                const getRealAcc = (numAcc) => {
                    ...
                    // 绝对模式：直接使用指定的位数，但不能超过数据本身的精度(可选的安全限制)
                    return Math.min(acc, numAcc); // 建议加上这行，防止用户要求输出精度高于计算精度
                };
            ...
            static getTokenInfo(token) {
                ...
                function getOrder(token) {
                    ...
                    // l7: 加法和减法
                    const levels = {
                        l0: [...func_01_2, ...complement(func_01_1, ['N'])], // 函数，如 log, sin 等
                        l1: func_10_1, // 后缀运算符，如 !
                        l2: ['^'], // 幂运算
                        l3: ['N'], // 一元负号
                        l4: ['&'], // 隐式乘法
                        l5: complement(func_11_2, ['+', '-', '*', '&', '/', 'mod', '^']), // 其他二元运算符
                        l6: ['*', '/', 'mod'], // 乘、除、模
                        l7: ['+', '-'] // 加、减
                    };
                    // 获取枚举属性组成的数组
        ...
        class CalcConfig {
             ...
             * @readonly
             * @type {number}
             * @description 在 `MathPlus.pow()` 中，当指数为整数时，允许的结果的最大十进制位数。
             * 这是一个安全限制，用于在上溢发生前提前中止计算，防止因生成一个极大的数字而耗尽内存。
             */
            static MAX_POWER_ORDER_MAGNITUDE = 2e7;
            ...
            static constants = Object.freeze({
                ...
                // 2*pi，用于三角函数的周期性计算，具有更高的精度以减少累积误差。
                super_2pi: [-439, , 440],
                // ln(10)，用于对数函数的换底计算。
