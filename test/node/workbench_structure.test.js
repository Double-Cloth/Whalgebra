...
test("入口、工具和测试页面共享公共 UI 资源且不包含内联实现", async () => {
    ...
    assert.doesNotMatch(logConsoleJs, /console\.(?:log|warn|error|info)\s*=/u);
    assert.match(testConsoleJs, /createLogConsole/u);
    assert.match(testConsoleJs, /createTestOutputLogger/u);
    assert.match(testConsoleJs, /logger: testOutputLogger/u);
    assert.doesNotMatch(logConsoleJs + testConsoleJs, /enableConsoleCapture|disableConsoleCapture|originalConsole/u);
