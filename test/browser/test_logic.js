...
async function test(mode = 0, engineFrame = document.getElementById("logicEngine")) {
    ...
    switch (mode) {
        ...
        case 2: { // Power
            let allPass = true;
            const cases = await fetchJson('../cases/test_power_function.json');
        ...
        case 3: { // Stats
            let allPass = true;
            const cases = await fetchJson('../cases/test_statistics.json');
            for (let i = 0; i < cases.length; i++) {
                const c = cases[i];
        ...
        case 4: { // Radical
            let allPass = true;
            const cases = await fetchJson('../cases/test_radical_function.json');
            for (let i = 0; i < cases.length; i++) {
                const c = cases[i];
        ...
        case 5: { // ValueList
            let allPass = true;
            const cases = await fetchJson('../cases/test_func_value_list.json');
            for (let i = 0; i < cases.length; i++) {
                const c = cases[i];
        ...
        case 6: { // Expr
            let allPass = true;
            const cases = await fetchJson('../cases/test_calc_expr.json');
            for (let i = 0; i < cases.length; i++) {
                const c = cases[i];
                const opts = {
                    calcAcc: c.coeffs[1],
        ...
        default: {
            ...
            for (let i = 2; i <= 6; i++) {
                console.info(`\n============== [${i}] ${sections[i]} ==============`);
                const subResult = await test(i, engineFrame);
